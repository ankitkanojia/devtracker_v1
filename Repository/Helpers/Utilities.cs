    ...
    public static class Utilities
    {
        ...
        public static string GenerateHash(string password, string username, byte[] salt)
        {
            try
            {
                ...
                //Plain Text in Byte
                var plainTextBytes = Encoding.UTF8.GetBytes(password + username);
                ...
                for (var i = 0; i < salt.Length; i++) plainTextWithSaltBytes[plainTextBytes.Length + i] = salt[i];
                ...
                for (var i = 0; i < salt.Length; i++) hashWithSaltBytes[hashBytes.Length + i] = salt[i];
        ...
        public static bool CompareHashValue(string password, string email, string oldHashValue, byte[] salt)
        {
            try
            {
                var expectedHashString = GenerateHash(password, email, salt);
