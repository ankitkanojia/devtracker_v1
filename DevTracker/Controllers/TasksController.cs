    ...
    public class TasksController : Controller
    {
        ...
        public async Task<ActionResult> Create(long id = 0)
        {
            using (_entities = new DBEntities())
            {
                ...
                {
                    taskVm.Description =
                        "<p><strong>Objective:</strong></p><p>&nbsp;</p><p><strong>References:</strong></p><p>&nbsp;</p><p><strong>Condition Satisfied:</strong></p>";
                }
        ...
        public async Task<ActionResult> _GetComments(long taskMasterId)
        {
            using (_entities)
            {
                var commentMaster = await _entities.CommentMasters
                    .Where(s => s.TaskMasterId == taskMasterId && !s.IsDelete).OrderByDescending(s => s.CreatedDate)
                    .Take(10).ToListAsync();
        ...
        public async Task<ActionResult> _GetActivities(long taskMasterId)
        {
            using (_entities)
            {
                var activities = await _entities.Activities.Where(s => s.TaskMasterId == taskMasterId && !s.IsDelete)
                    .OrderByDescending(s => s.CreatedDate).Take(10).ToListAsync();
